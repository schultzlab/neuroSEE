...
function GUI_manually_refine_spikes( origspikes, tsG, dtsG, df_f, ddf_f, data_locn, file, origparams, corr_image, masks )
...
if nargin < 10, masks = []; end 
if nargin < 9, corr_image = zeros(512,512); end
...
function pbutton_save_callback(varargin)
    ...
    output.tsG = tsG;
    if ~isempty(dtsG), output.dtsG = dtsG; end
    if ~isempty(df_f), output.df_f = df_f; end
    if ~isempty(ddf_f), output.ddf_f = ddf_f; end

    if ~get(rbutton_autoprctile,'Value')
        output.params.bl_prctile = str2double(get(edit_prctile,'string'));
    else
        output.params.bl_prctile = getappdata(fig_gui,'auto_prctile');
    end
    output.params.spk_SNR = str2double(get(edit_spikeSNR,'string'));
    output.params.decay_time = str2double(get(edit_decaytime,'string'));
    output.params.lam_pr = str2double(get(edit_lamprob,'string'));
    params = origparams;
    params.spkExtract = output.params;

    mcorr_method = origparams.methods.mcorr_method;
    segment_method = origparams.methods.segment_method;
    if origparams.methods.dofissa
        str_fissa = 'FISSA';
    else
        str_fissa = 'noFISSA';
    end
    ...
    setappdata(fig_gui,'saved_spikes',output.spikes);
