...
for jj = 1:numel(idx_file)-1
    ...
    for k = 1:numel(idx_tr)-1
        ...
        spike{Ntrial} = s(idx_tr(k)+1:idx_tr(k+1));
        Ntrial = Ntrial + 1;
    end
...
for tr = 1:numel(phi)
    ...
    for n = 1:Nbins
        spkMap_pertrial(tr,n) = sum(spike_tr(phi_tr == n));
    end

    normspkMap_pertrial(tr,:) = spkMap_pertrial(tr,:)./max(spkMap_pertrial(tr,:));
    normspkMap_pertrial(isnan(normspkMap_pertrial)) = 0;
end
