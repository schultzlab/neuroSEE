...
Names = [
    ...
    % ROI segmentation (general)
        ...
        'roiareamin_FOV490  ' % min roi area for cells, 490x490 um FOV (default: 50)
        'roiareamax_FOV490  ' % max roi area for cells, 490x490 um FOV (default: 400)
        'roiareamin_FOV330  ' % min roi area for cells, 330x330 um FOV (default: 70)
        'roiareamax_FOV330  ' % max roi area for cells, 330x330 um FOV (default: 560)
        'invcirc_max        ' % max inverse circularity for cells (default: 4)
        'overlap_thr        ' % max allowed overlap between any two cells (default: 0.25)
    ...
    % ROI segmentation: CaImAn
        ...
        'tsub               ' % temporal downsampling factor (default: 10)
        ...
        'min_corr           ' % minimum local correlation for initializing a neuron5* (default: 0.3)
...
Values = [
    ...
    % ROI segmentation
        ...
        {50}                  % min roi area for cells, 490x490 um FOV (default: 50)
        {400}                 % max roi area for cells, 490x490 um FOV (default: 400)
        {70}                  % min roi area for cells, 330x330 um FOV (default: 70)
        {560}                 % max roi area for cells, 330x330 um FOV (default: 560)
        {4}                   % max inverse circularity for cells (default: 4)
        {0.25}                % max allowed overlap between any two cells (default: 0.25)
    % ROI segmentation (ABLE)
    ...
    % ROI segmentation: CaImAn
        ...
        {10}                   % temporal downsampling factor (default: 10)
...
params.ROIsegment.invcirc_max = options.invcirc_max;
params.ROIsegment.overlap_thr = options.overlap_thr;
if options.FOV == 330
    maxcells = options.maxcells_FOV330;
    cellrad = options.cellrad_FOV330;
    params.ROIsegment.roiarea_min = options.roiareamin_FOV330;
    params.ROIsegment.roiarea_max = options.roiareamax_FOV330;    
else
    maxcells = options.maxcells_FOV490;
    cellrad = options.cellrad_FOV490;
    params.ROIsegment.roiarea_min = options.roiareamin_FOV490;
    params.ROIsegment.roiarea_max = options.roiareamax_FOV490;    
end
