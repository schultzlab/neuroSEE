...
% Edit by Ann Go: lines 81, 84, 89-93
...
function imData = read_file( path_to_file, sframe, num2read, options, im_info )
   ...
   elseif strcmpi(ext,'.raw')
       if nargin < 4 || ~isfield(options,'d1') || isempty(options.d1)
           options.d1 = 512; %input('What is the total number of rows? \n');
       end
       if nargin < 4 || ~isfield(options,'d2') || isempty(options.d2)
           options.d2 = 512; %input('What is the total number of columns? \n');
       end
       if nargin < 4 || ~isfield(options,'bitsize') || isempty(options.bitsize)
           options.bitsize = 2;
       end
       if nargin < 4 || ~isfield(options,'skipN') || isempty(options.skipN)
           options.skipN = 1;
       end   

       imData = read_raw_file(path_to_file,sframe,num2read,[options.d1,options.d2],options.bitsize,options.skipN);
   else
