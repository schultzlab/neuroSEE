...
function GUI_viewSpikeRaster(list, reffile, concrunsrois, dofissa)
if nargin<4, dofissa = false; end
    if dofissa
        str_fissa = 'FISSA';
    else
        str_fissa = 'noFISSA';
    end
if nargin<3, concrunsrois = true; end
...
grp_sdir = [grp_sdir str_fissa '/'];

% data
fname = [grp_sdir mouseid '_' expname '_ref' reffile '_PFmap_output.mat'];        
data = load(fname);
raster = data.PFdata.normspkRaster;
ytick_files = data.PFdata.ytick_files;
Nrois = numel(raster);
