...
function frun_pipeline_imreg_test( list, reffile, dofissa, force, dostep, tsub, bl_prctile, prctile_thr, pfactivet_thr, activetrials_thr, min_SNR )
    ...
    if force(5) || ~check_list(4)
        ...
        for n = 1:Nfiles
            ...
            if force(5) || ~exist([file_sdir file '_downTrackdata.mat'],'file')
                ...
                if any(cdownTrackdata{n} < 100) || ~isfield(cdownTrackdata{n},'zone')
                    trackfile = findMatchingTrackingFile( data_locn, file, true );
                    c = load_trackfile(data_locn, files(n,:), trackfile, true);
                    downTrackdata = downsample_trackData( c, framesperfile(n), params.PFmap.fr );
                    save([file_sdir file '_downTrackdata.mat'],'downTrackdata');
                    cdownTrackdata{n} = downTrackdata;
                end
            ...
            SdownTrackdata.zone = [SdownTrackdata.zone; cdownTrackdata{n}.zone];
        end
        clear downTrackdata
...
if dostep(6)
    if any(downTrackdata.r < 100)
        if max(downTrackdata.zone) < 9
            params.mode_dim = '2D';                 % open field
        else
            params.mode_dim = 'Y';                  % Y-maze
        end
