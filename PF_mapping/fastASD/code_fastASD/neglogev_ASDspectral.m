...
% Compute neglogli d
try
    px=XX+Cinv;px(isnan(px))=0;px(isinf(px))=0; % make sure no nan or inf
    trm1 = -.5*(logdet(px) + sum(log(cdiag)) + (dd.nsamps)*log(2*pi*nsevar)); % Log-determinant term
catch
    disp('Fourier coefficients matrix is not positive definite, performing approximation')
    spx = matPosApprox(px,1e-8);
    trm1 = -.5*(logdet(spx) + sum(log(cdiag)) + (dd.nsamps)*log(2*pi*nsevar)); % Log-determinant term
end
...
function X = matPosApprox(x,eps)
    X = x+eps*eye(length(x));
end
