function [M_final_Y,M_final_X,shifts_g,template,options,col_shift] = normcorre_batch_2ch(Y,X,options,template)
...
if nargin < 4 || isempty(template)
    fprintf('\tRegistering the first %i frames just to obtain a good template....\n',init_batch);
...
switch lower(options.output_type)
    case 'mat'
        M_final_Y = zeros([sizY,T],data_type);
        M_final_X = zeros([sizY,T],data_type); %%%
    case 'memmap'
        M_final_Y = matfile(filename,'Writable',true);
        if nd == 2; M_final_Y.Y(d1,d2,T) = zeros(1,data_type); end
        if nd == 3; M_final_Y.Y(d1,d2,d3,T) = zeros(1,data_type); end
        M_final_Y.Yr(d1*d2*d3,T) = zeros(1,data_type);        
    case {'hdf5','h5'}
    ...
    case {'tif','tiff'}
        M_final_Y = ['motion corrected file has been saved as ', options.tiff_filename];
...
%                 end
            ...
            case 'mat'
                ...
                if nd == 3
                    ...
                    Xtm = single(X(:,:,:,t:min(t+bin_width-1,T))); %%%
                end
            case 'raw'
        ...
        if nd == 3 
            ...
            Xtc = mat2cell(Xtm,d1,d2,d3,ones(1,size(Xtm,ndims(Xtm)))); %%%green channel
        end
        Mf_y = cell(size(Ytc));
        Mf_x = cell(size(Xtc)); %%%
        lY = length(Ytc);
        ...
        parfor ii = 1:lY
                ...
                for j = 1:length(yy_s)           
                    for k = 1:length(zz_s)
                        ...
                        if all([length(xx_s),length(yy_s),length(zz_s)] == 1) && strcmpi(options.shifts_method,'fft');
                            M_fin_y{i,j,k} = shift_reconstruct(Yt,shifts_temp(i,j,k,:),diff_temp(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value);
                        end                                               
                    end
                end
            end            
            ...
            switch lower(options.shifts_method)
                case 'fft'
                        ...
                        for i = 1:length(xx_uf)
                            for j = 1:length(yy_uf)
                                for k = 1:length(zz_uf)
                                    extended_grid = [max(xx_us(i)-overlap_post(1),1),min(xx_uf(i)+overlap_post(1),d1),max(yy_us(j)-overlap_post(2),1),min(yy_uf(j)+overlap_post(2),d2),max(zz_us(k)-overlap_post(3),1),min(zz_uf(k)+overlap_post(3),d3)];
                                    I_temp_y = Yt(extended_grid(1):extended_grid(2),extended_grid(3):extended_grid(4),extended_grid(5):extended_grid(6));
                                    I_temp_x = Xt(extended_grid(1):extended_grid(2),extended_grid(3):extended_grid(4),extended_grid(5):extended_grid(6)); %%%
                                    M_fin_y{i,j,k} = shift_reconstruct(I_temp_y,shifts_up(i,j,k,:),diff_up(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value);
                                    M_fin_x{i,j,k} = shift_reconstruct(I_temp_x,shifts_up(i,j,k,:),diff_up(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value); %%%
                    ...
                    if flag_interp    
                        Mf_y{ii} = cell2mat_ov_sum(M_fin_y,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY,Bs) - add_value;
                        Mf_x{ii} = cell2mat_ov_sum(M_fin_x,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY,Bs) - add_value;
                    else            
                        Mf_y{ii} = cell2mat_ov(M_fin_y,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                        Mf_x{ii} = cell2mat_ov(M_fin_x,xx_us,xx_uf,yy_us,yy_uf,zz_us,zz_uf,overlap_post,sizY) - add_value;
                    end                    
                ...
                otherwise
                    ...
                    if nd == 3                
                        ...
                        Mf_y{ii} = imwarp(Yt,-cat(4,shifts_up(:,:,:,2),shifts_up(:,:,:,1),shifts_up(:,:,:,3)),options.shifts_method,'FillValues',fill_value);
                        Mf_x{ii} = imwarp(Xt,-cat(4,shifts_up(:,:,:,2),shifts_up(:,:,:,1),shifts_up(:,:,:,3)),options.shifts_method,'FillValues',fill_value);
                    else
                        shifts_up = imresize(shifts_temp,[options.d1,options.d2]);
                        shifts_up(2:2:end,:,2) = shifts_up(2:2:end,:,2) + col_shift;
                        Mf_y{ii} = imwarp(Yt,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value);
                        Mf_x{ii} = imwarp(Xt,-cat(3,shifts_up(:,:,2),shifts_up(:,:,1)),options.shifts_method,'FillValues',fill_value);
                    end   
            end
            Mf_y{ii}(Mf_y{ii}<minY)=minY;
            Mf_y{ii}(Mf_y{ii}>maxY)=maxY;
            Mf_x(Mf_x<minX) = minX;
            Mf_x(Mf_x>maxX) = maxX;
        end
        ...
        if ~strcmpi(options.output_type,'mat')
            ...
            if nd == 2; mem_buffer(:,:,rem_mem-lY+1:rem_mem) = cast(Mf_y,data_type); end
            if nd == 3; mem_buffer(:,:,:,rem_mem-lY+1:rem_mem) = cast(Mf_y,data_type); end
        end
        if it == iter
        switch lower(options.output_type)
            case 'mat'
                if nd == 2
                    M_final_Y(:,:,t:min(t+bin_width-1,T)) = cast(Mf_y,data_type); 
                    M_final_X(:,:,t:min(t+bin_width-1,T)) = cast(Mf_x,data_type);
                end
                if nd == 3 
                    M_final_Y(:,:,:,t:min(t+bin_width-1,T)) = cast(Mf_y,data_type); 
                    M_final_X(:,:,:,t:min(t+bin_width-1,T)) = cast(Mf_x,data_type);
            ...
            case 'memmap'
                if rem_mem == options.mem_batch_size || t+lY-1 == T
                    if nd == 2; M_final_Y.Y(:,:,t+lY-rem_mem:t+lY-1) = mem_buffer(:,:,1:rem_mem); end
                    if nd == 3; M_final_Y.Y(:,:,:,t+lY-rem_mem:t+lY-1) = mem_buffer(:,:,:,1:rem_mem); end
                    M_final_Y.Yr(:,t+lY-rem_mem:t+lY-1) = reshape(mem_buffer(1:d1*d2*d3*rem_mem),d1*d2*d3,rem_mem);
                end      
            case {'hdf5','h5'}
                if rem_mem == options.mem_batch_size || t+lY-1 == T
        ...
        if upd_template
            cnt_buf = cnt_buf + 1;
            if nd == 2; buffer = mat2cell_ov(Mf_y,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end
            if nd == 3; buffer = mat2cell_ov(Mf_y,xx_s,xx_f,yy_s,yy_f,zz_s,zz_f,overlap_pre,sizY); end        
