...
switch filetype
    case 'tif'
        ch1_temp= read_file(Y,interval(1),init_batch,[],tiffInfo);
    case 'hdf5'
        ch1_temp= read_file(Y,interval(1),init_batch);        
    case 'mem'
        if nd == 2; ch1_temp= Y.(var_name)(:,:,interval); elseif nd == 3; ch1_temp= Y.(var_name)(:,:,:,interval); end
    case 'mat'
        if nd == 2; ch1_temp= Y(:,:,interval); elseif nd == 3; ch1_temp= Y(:,:,:,interval); end
end

data_type = class(Y_temp);
ch1_temp= single(Y_temp);
...
if plot_flag
    ...
    if strcmpi(filetype,'mat')
        nnY = quantile(ch1(:),0.005);
        mmY = quantile(ch1(:),0.995);
    else
        nnY = quantile(ch1_temp(:),0.005);
        mmY = quantile(ch1_temp(:),0.995);
    end
    fig = figure;
