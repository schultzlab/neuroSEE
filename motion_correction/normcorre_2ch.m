...
% col_shift:        relative shift due to bi-directional scanning
   ...
   switch lower(options.output_type)
       ...
       case {'tif','tiff'}
           M_final_y = options.tiff_filename;
       ...
       for t = 1:T
           switch filetype
               case 'tif'
                   ...
                   Xt = single(imread(X,'Index',t,'Info',tiffInfo)); %%%
               case 'hdf5'
                   Yt = single(h5read(Y,'/mov',[ones(1,nd),t],[sizY(1:nd),1]));
                   Xt = single(h5read(X,'/mov',[ones(1,nd),t],[sizY(1:nd),1])); %%%
               case 'mem'
                   if nd == 2
                       Yt = single(Y.(var_name)(:,:,t)); 
                       Xt = single(X.(var_name2)(:,:,t)); 
                   end
                   if nd == 3
                       Yt = single(Y.(var_name)(:,:,:,t)); 
                       Xt = single(X.(var_name2)(:,:,:,t)); 
           ...
           M_fin_x = cell(length(xx_us),length(yy_us),length(zz_us)); %%%
                   ...
                   for j = 1:length(yy_s)           
                       for k = 1:length(zz_s)
                           ...
                           if all([length(xx_s),length(yy_s),length(zz_s)] == 1)
                               M_fin_y{i,j,k} = remove_boundaries(M_temp,output(3:end),options.boundary,template{i,j,k},add_value);
                           end                                               
                       end
                   end
               end      
   ...
   %             end
                       ...
                       for i = 1:length(xx_uf)
                           for j = 1:length(yy_uf)
                               for k = 1:length(zz_uf)
                                   ...
                                   I_temp2 = Xt(extended_grid(1):extended_grid(2),extended_grid(3):extended_grid(4),extended_grid(5):extended_grid(6)); %%%
                                   M_fin_y{i,j,k}  = shift_reconstruct(I_temp,shifts_up(i,j,k,:),diff_up(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value); 
                                   M_fin_x{i,j,k} = shift_reconstruct(I_temp2,shifts_up(i,j,k,:),diff_up(i,j,k),us_fac,Nr{i,j,k},Nc{i,j,k},Np{i,j,k},options.boundary,add_value);  %%%
           ...
           switch lower(options.output_type)
               ...
               case 'memmap'
                   if rem_mem == options.mem_batch_size || t == T
                       if nd == 2; M_final_y.Y(:,:,t-rem_mem+1:t) = mem_buffer(:,:,1:rem_mem); end
                       if nd == 3; M_final_y.Y(:,:,:,t-rem_mem+1:t) = mem_buffer(:,:,:,1:rem_mem); end
                       M_final_y.Yr(:,t-rem_mem+1:t) = reshape(mem_buffer(1:d1*d2*d3*rem_mem),d1*d2*d3,rem_mem);
                   end      
               case {'hdf5','h5'}
                   if rem_mem == options.mem_batch_size || t == T
           ...
           if plot_flag && mod(t,1) == 0
               ...
               subplot(222); imagesc(Mf_y,[nnY,mmY]);  title('Motion Corrected','fontweight','bold','fontsize',14); colormap('bone'); axis off;
