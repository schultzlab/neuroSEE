...
for n = 1:Nmasks
    masks{n} = rawmasks.PixelIdxList{n};
end
...
for n = 1:size(files,1)
    ...
    cprintf(str)
    load([filedir files(n,:) '/' files(n,:) '_2P_mcorr_output.mat'])
    open(n).green = green;
    open(n).red = red;
    clear green red params

    open_imG = read_file( [filedir files(n,:) '/' files(n,:) '_2P_XYT_green_mcorr.tif'] );
    open_imR = read_file( [filedir files(n,:) '/' files(n,:) '_2P_XYT_red_mcorr.tif'] );

    [ open(n).shift, open(n).red.globalreg_meanframe ] = globalregisterImage(ref.red.meanregframe, open(n).red.meanregframe, 1 );
    [ open_imGglobalreg, open_imRglobalreg ] = ...
        globalregisterStack( open_imG, open_imR, open(n).shift(1) , open(n).shift(2) );
    clear open_imG open_imR
    fname_tif_open_imGglobalreg = [filedir files(n,:) '/' files(n,:) '_2P_XYT_green_mcorr_globalreg.tif'];
        writeTifStack( open_imGglobalreg, fname_tif_open_imGglobalreg );
    fname_tif_open_imRglobalreg = [filedir files(n,:) '/' files(n,:) '_2P_XYT_red_mcorr_globalreg.tif'];
        writeTifStack( open_imRglobalreg, fname_tif_open_imRglobalreg );

    for i = 1:Nmasks
        maskind = masks{i};
        for j = 1:size(open_imGglobalreg,3)
            imG_reshaped = reshape( open_imGglobalreg(:,:,j), 512*512, 1);
            open(n).tsG( i, j ) = mean( imG_reshaped(maskind) );
            imR_reshaped = reshape( open_imRglobalreg(:,:,j), 512*512, 1);
            open(n).tsR( i, j ) = mean( imR_reshaped(maskind) );
        end
    end
    clear open_imGglobalreg open_imRglobalreg imG_reshaped imR_reshaped maskind i j

    open(n).R = ratiometric_Ca( open(n).tsG, open(n).tsR, 11 );
    open(n).spikes = nndORoasis(open(n).R, 2, 0.94, 2.4);
end

clear Nmasks data_locn filedir files refIm str n fname_tif_open_imGglobalreg fname_tif_open_imRglobalreg
...
save('m82_openfield_20190406.mat')
