...
function frun_CaImAn_expbatch( list, reffile, slacknotify )

if nargin<3, slacknotify = false; end
% if nargin<2, see line 37
...
for i = 1:size(files,1)
    file = files(i,:);
    if strcmpi(file,reffile)
        sdir = [data_locn 'Data/' file(1:8) '/Processed/' file '/mcorr' mcorr_method '/' segment_method '_' mouseid '_' expname '/'];
        if ~exist(sdir,'dir'), mkdir(sdir); end
        sname = [sdir mouseid '_' expname '_ref' reffile '_masks.mat'];
    else
        sdir = [data_locn 'Data/' file(1:8) '/Processed/' file '/mcorr' mcorr_method '_ref' reffile '/' segment_method '_' mouseid '_' expname '/'];
        if ~exist(sdir,'dir'), mkdir(sdir); end
        sname = [sdir mouseid '_' expname '_ref' reffile '_masks.mat'];
    end
