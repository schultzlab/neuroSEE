...
function [ sorted_pfMap, sortIdx ] = sortPFmap_1d( pfMap, infoMap )
    ...
    for e = 1:Nepochs
        ...
        % sort by information
        [~,idx] = sort( squeeze(infoMap(:,:,e)) );                      
        idx_info = idx( round(n_info*Npcs)+1:Npcs );                    % info idx (<Ncells x 1)
        idx_no = idx( 1:round(n_info*Npcs) );                           % no info idx (<Ncells x 1)
