...
%% SI (bits/sec)
    ...
    if ~isempty(hist.SIsec.pcIdx)
        ...
        if ~isempty(asd)
            plotpfMaps_2d(activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,1), asd.normrMap, asd.infoMap(:,1), ...
                            hist.SIsec.pcIdx, 'PC', 'bits/s', fname, fclose);
        else
            plotpfMaps_2d(activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,1), [], [], ...
                            hist.SIsec.pcIdx, 'PC', 'bits/s', fname, fclose);
        end
    ...
    if ~isempty(hist.SIsec.nonpcIdx)
        ...
        if ~isempty(asd)
            plotpfMaps_2d(activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,1), asd.normrMap, asd.infoMap(:,1), ...
                            hist.SIsec.nonpcIdx, 'NPC', 'bits/s', fname, fclose);
        else
            plotpfMaps_2d(activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,1), [], [], ...
                            hist.SIsec.nonpcIdx, 'NPC', 'bits/s', fname, fclose);
        end
        ...
        plot_pfChar( Nnonpcs, hist.normrMap_sm(:,:,hist.SIsec.nonpcIdx), hist.centroid(hist.SIsec.nonpcIdx), 'NPC', fname, fclose );
...
%% SI (bits/spike)
    ...
    if ~isempty(hist.SIspk.pcIdx)
        ...
        if ~isempty(asd)
            plotpfMaps_2d(activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,2), asd.normrMap, asd.infoMap(:,2), ...
                            hist.SIspk.pcIdx, 'PC', 'bits/event', fname, fclose);
        else
            plotpfMaps_2d(activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,2), [], [], ...
                            hist.SIspk.pcIdx, 'PC', 'bits/event', fname, fclose);
        end
    ...
    if ~isempty(hist.SIspk.nonpcIdx)
        ...
        if ~isempty(asd)
            plotpfMaps_2d( activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,2), asd.normrMap, asd.infoMap(:,2), ...
                            hist.SIspk.nonpcIdx, 'NPC', 'bits/event', fname, fclose );
        else
            plotpfMaps_2d( activeData, hist.rMap, hist.normrMap, hist.normrMap_sm, hist.infoMap(:,2), [], [], ...
                            hist.SIspk.nonpcIdx, 'NPC', 'bits/event', fname, fclose);
        end
        ...
        plot_pfChar( Nnonpcs, hist.normrMap_sm(:,:,hist.SIspk.nonpcIdx), hist.centroid(hist.SIspk.nonpcIdx), 'NPC', fname, fclose );
